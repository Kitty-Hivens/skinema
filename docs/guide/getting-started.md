...
```kotlin
implementation("dev.hivens:skinema-compose:0.8.2")   // brings -core and -skiko
...
soname the loader asks for is what ties a library release to a bundle: 0.8.2
is the n9.0 line, and an FFmpeg 8 bundle will not resolve against it.

0.8.2 compiles against Skiko 0.150.1, the copy Compose Multiplatform 1.12.0
...
- A playlist: [video-player.md](video-player.md) -- `setSource` puts the
  next file on the player you already have, so the queue and its order
  stay yours.
