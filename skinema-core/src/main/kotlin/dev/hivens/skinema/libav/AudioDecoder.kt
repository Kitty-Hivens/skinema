...
class AudioDecoder private constructor(
    ...
    companion object {
        ...
        /** Opens [path]'s best audio stream; null when the file has none. */
        fun openOrNull(path: Path): AudioDecoder? {
            ...
            try {
                ...
                val streamIndex = Libav.avFindBestStream(fmtCtx, LibavAbi.AVMEDIA_TYPE_AUDIO, decoderOut)
                if (streamIndex < 0) {
                    // No audio stream (or no decoder for it): a silent file,
                    // not a failure.
                    val ptrPtr = arena.allocate(ADDRESS)
                    ptrPtr.set(ADDRESS, 0, fmtCtx)
                    Libav.avformatCloseInput(ptrPtr)
