...
- Pure units: pacer, pts/rational math, frame mailbox, soname table --
  no natives involved.
- Golden-frame integration tests: tiny fixture videos generated by the
  ffmpeg CLI in CI (not committed binaries), decoded through the real
  bindings, pixel-checksummed against goldens. Decode is deterministic
  computation -- unlike GUI/OS-integration native code, it is genuinely
  CI-testable, including on macOS runners (both archs) standing in for
...
- Core API shape: suspend/Flow (coroutines dependency in core) vs
  callback/blocking with coroutine adapters in skinema-compose. Decide
  during M1 against the consumer.
