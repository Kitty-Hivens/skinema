...
- **M20 -- closed captions, and the path that is not a stream (2026-08-29).**
  Issue #11 tracked three formats and they turned out to be three different
  answers. DVB shipped with the subtitle work and the issue had simply gone
  stale on it. TTML is not reachable: FFmpeg 9.0.1 has a TTML encoder and
  muxer and NO decoder, so reading one would mean an XML parser and a text
  layout of our own rather than a whitelist entry -- a subsystem, for a format
  the consumer's inputs do not contain. CEA-608/708 is the one that was worth
  building, and the issue's own note said why it would be different: it rides
  inside the video bitstream rather than beside it.

  So the whitelist entry is one decoder, `ccaption`, with no demuxer and no
  parser. The h264 and hevc decoders extract A53 SEI unconditionally -- core
  behaviour, not a build option -- and hang it off each decoded frame, so what
  the bundle has to carry is only the thing that turns those bytes into cues.
  It selects the ASS infrastructure the text tracks already bring and emits
  ASS, so the render path is the one that exists. Its capability is named
  separately (`cea608`) for the reason dvbsub's is: a bundle built before it is
  a legal bundle, and folding it into the subtitle contract would turn "older
  than this" into a red row everywhere subtitles are mandatory.

  **The fixture is the part with no precedent.** Every other fixture here is
  generated by the ffmpeg CLI, and this one cannot be: FFmpeg has no CEA-608
  encoder anywhere, so there is no path from a command line to a file with
  captions in it. The generator writes the bytes into an x264 bitstream itself
  -- A53 user data in its registered ITU-T T.35 form, cc_data triplets on
  field one, odd parity where the format checks it, emulation prevention over
  the payload, and the SEI placed before the coded picture it belongs to. It
  is verified by something that is not it: the test reads the result back
  through FFmpeg's own caption path (the movie filter's subcc output), so a
  fixture only this code agrees with fails. Mutation-checked against three
  things the format itself checks -- parity, the ATSC identifier, and
  process_cc_data_flag -- each of which leaves FFmpeg reading nothing.

  Discovery is LAZY, and that is a decision rather than an omission. No
  container can be asked whether a file has captions, so the track appears
  when the first payload is seen and never otherwise. The alternatives were
  both worse: probing N frames at open charges every file for a feature almost
  none of them have and still cannot promise anything, since captions may
  start later than the probe; offering the track for every H.264 file
  advertises something that would render an empty screen. The shape it does
  take is the one `durationNanos` already takes on an animated webp.

  The pipeline grew a mode rather than a sibling. A caption session opens no
  format context, seeks no demuxer and has no horizon -- the decode thread
  hands payloads over and paces it by construction. The queue between them is
  bounded and DROPPING, because the producer is the thread that paces the
  picture and a subtitle side that fell behind must never become back-pressure
  on it. A reposition flushes instead of seeking, since a preroll cannot
  replay what was never in a container.

  Two things the round cost that are worth keeping. The first full natives
  matrix since three merges found the core tier's skip ceiling stale -- the
  backstop working as designed, saying a suite had gone quieter than the
  tier's shape explained, because the shape had changed. And behind it sat a
  RACY test of mine: the canvas-announcement test asserted a caller-side
  property while depending on the pipeline being alive, which on a tier with no
  subtitle decoder it never is. It won that race on every linux row and lost on
